...
`;
      ...
      const corsSnippet = `if ($request_method = OPTIONS) { add_header 'Access-Control-Allow-Origin' '$http_origin' always; add_header 'Vary' 'Origin' always; add_header 'Access-Control-Allow-Methods' 'GET, HEAD, POST, PUT, PATCH, DELETE, OPTIONS' always; add_header 'Access-Control-Allow-Headers' 'authorization, apikey, content-type, x-client-info, x-upsert, prefer, accept-profile, content-profile, range, x-requested-with' always; add_header 'Access-Control-Max-Age' 86400 always; add_header 'Content-Length' 0 always; return 204; } add_header 'Access-Control-Allow-Origin' '$http_origin' always; add_header 'Vary' 'Origin' always; add_header 'Access-Control-Expose-Headers' 'content-range, x-supabase-api-version, x-request-id, location' always;`;
      ...
      const localFunctionLocations = localFunctions.map((name) => `  location = /functions/v1/${name} { ${corsSnippet} proxy_pass http://host.docker.internal:${supaKongPort}/functions/v1/${name}; ${functionProxyHeaders} }`).join("\n");
      ...
      const commonProxyHeaders = (proto: string) => `${corsSnippet} proxy_set_header Host $host; proxy_set_header Authorization $http_authorization; proxy_set_header apikey $http_apikey; proxy_set_header X-Client-Info $http_x_client_info; proxy_set_header X-Upsert $http_x_upsert; proxy_set_header Content-Type $http_content_type; proxy_set_header X-Forwarded-Proto ${proto}; proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;`;
